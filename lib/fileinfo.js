...
FileInfo.prototype.safeName = function() {
  ...
  if (this.req.headers['content-range']) {
    lastFileName = decodeURIComponent(this.req.headers['content-disposition'].replace(/(^[^"]+")|("$)/,'').replace('"',''));
    firstIndex = this.req.headers['content-range'].replace('bytes ','').split(/[^\d]+/)[0] == 0;
  };
  ...
  if (!this.req.headers['content-range']||firstIndex) {
    fileName = lastFileName;
  };
